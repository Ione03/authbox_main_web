                        ...
                        <div class="flex w-full items-center justify-between px-4">
                            <div>
                                ...
                                >
                                    ...
                                    <div class="mt-4 flex flex-col gap-3 border-t border-stroke pt-4 dark:border-dark-3 lg:hidden">
                                        {/* Search */}
                                        <button
                                            onClick$={() => { searchOpen.value = true; navbarOpen.value = false; }}
                                            class="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-body-color transition hover:bg-gray-50 dark:text-dark-6 dark:hover:bg-dark-3"
                                        >
                                        ...
                                        {/* Notifications */}
                                        <button class="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-body-color transition hover:bg-gray-50 dark:text-dark-6 dark:hover:bg-dark-3">
                                            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                                            </svg>
                                            Notifications
                                            <span class="ml-auto rounded-full bg-red-500 px-2 py-0.5 text-[10px] font-bold text-white">2</span>
                                        </button>
                                        {/* Theme toggle */}
                                        <div class="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-body-color dark:text-dark-6">
                                            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" />
                                            </svg>
                                            Dark Mode
                                            <div class="ml-auto"><ThemeToggler /></div>
                                        </div>
                                        {/* Auth */}
                                        {session.value?.user ? (
                                            <div class="flex flex-col gap-2 border-t border-stroke pt-3 dark:border-dark-3">
                                                <div class="flex items-center gap-3 px-3 py-2">
                                                    {session.value.user.image ? (
                                                        <img src={session.value.user.image} alt={session.value.user.name || "User"} class="h-8 w-8 rounded-full ring-2 ring-stroke dark:ring-dark-3" />
                                                    ) : (
                                                        <div class="flex h-8 w-8 items-center justify-center rounded-full bg-primary/10 text-xs font-bold text-primary dark:bg-primary/20">
                                                            {(session.value.user.name || 'U').charAt(0).toUpperCase()}
                                                        </div>
                                                    )}
                                                    <span class="text-sm font-medium text-dark dark:text-white">{session.value.user.name}</span>
                                                </div>
                                                <button
                                                    onClick$={() => signOut.submit({ redirectTo: '/' })}
                                                    class="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-red-500 transition hover:bg-red-50 dark:hover:bg-red-900/20"
                                                >
                                                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            ...
                                            <a
                                                href="/signin"
                                                class="flex w-full items-center justify-center rounded-lg bg-primary px-5 py-2.5 text-sm font-medium text-white transition hover:bg-primary/90"
                                            >
                                                Login
                                            </a>
                                        )}
                            ...
                            <div class="hidden items-center justify-end gap-3 lg:flex">
                                ...
                                {session.value?.user ? (
                                    <div class="flex items-center gap-3">
                                        <a
                                            href="/dashboard"
                                            class="inline-flex items-center gap-1.5 rounded-md border border-amber-400 bg-amber-50 px-3 py-2 text-xs font-medium text-amber-700 shadow-sm transition hover:bg-amber-100 dark:border-amber-600 dark:bg-amber-900/20 dark:text-amber-400 dark:hover:bg-amber-900/40"
                                        >
                                        ...
                                        <div class="flex items-center gap-2.5">
                                            ...
                                            <span class={`text-sm font-medium transition-colors duration-300 ${sticky.value || !isHome ? "text-dark dark:text-white" : "text-white"}`}>
